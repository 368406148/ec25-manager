# EC25 Manager for macOS

一个面向移远 Quectel EC25 / Baiwang USB Modem 的 **macOS 菜单栏（menu bar）管理工具**，界面参照
[vohive](https://github.com/iniwex5/vohive-release) 的功能与观感，使用 Electron 构建 GUI，底层通过原生
...
- **概览**：美观的信号强度（信号格 + dBm）、运营商、网络制式（2G/3G/4G/5G）、注册状态、数据网络类型、
  IMEI、IMSI、ICCID、本机号码、SIM 状态、CS/PS/EPS 注册、分组附着、PDP 地址、频段/信道、载波聚合、服务小区
- **短信中心**：读取短信列表、UCS2 中文短信收发、删除短信
- **AT 终端**：内置终端 + 常用命令快捷标签
- **系统设置**：USB 网络模式切换（ECM / QMI / MBIM / RNDIS）、APN 配置、重新搜索网络、重启模块、重连设备
- 顶部实时连接状态，每 6 秒轮询刷新信号 / 注册信息
...
不同固件或运营商 SIM 的短信存储、字符集、收件号码格式可能略有差异；如某条命令失败，可在「实时日志 / AT 终端」中继续微调。
